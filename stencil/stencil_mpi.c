...
static void stencil_init(stencil *values, int size_x, int size_y, int rank, int size, int local_start, int local_end )
{
  int x,y;
  for(x = 0; x < size_x; x++)
    {
      for(y = local_start; y < local_end; y++)
        {
          values[x + size_x * (y - local_start)] = 0.0;
        }
    }
  ...
  if (rank == size - 1){
    for (x = 0; x<size_x; x++){
      values[x + size_x * (local_size -1)] = size_x - x;
    }
  }

  for (y = local_start; y<local_end; y++){
    values[0 + size_x * (y-local_start)] = y;
    values[size_x - 1 + size_x * (y - local_start)] = size_y - y;
  }
}
...
int main(int argc, char**argv)
{
  ...
  stencil_t *values = malloc(size_x * local_size * sizeof(stencil_t));
  stencil_t *prev_values = malloc(size_x * local_size * sizeof(stencil_t));

  
  stencil_init(values, size_x, local_size, rank, size, local_start, local_end); 
  memcpy(prev_values, values, size_x * local_size * sizeof(stencil_t));
